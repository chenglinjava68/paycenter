...
public class SecureUtil {
	...
	public static String fillString(String string, char filler,
		...
		for (int i = 0; i < delta; i++) {
			if (atEnd) {
				...
				localString = filler + localString;
			}
		}
		return string;
	...
	public static byte[] pin2PinBlockWithCardNO(String aPin, String aCardNO) {
		...
		String localACardNO = aCardNO;
		if (aCardNO.length() == 11) {
			localACardNO = "00" + aCardNO;
		} else if (aCardNO.length() == 12) {
			localACardNO = "0" + aCardNO;
		}
		byte[] tPanByte = formatPan(localACardNO);
		byte[] tByte = new byte[8];
		for (int i = 0; i < 8; i++) {
	...
	public String assymEncrypt(String tPIN, String iPan, RSAPublicKey publicKey) {
		...
		LogUtil.info(LOGGER,"SampleHashMap::assymEncrypt([" + tPIN + "])");
		LogUtil.info(LOGGER,"SampleHashMap::assymEncrypt(PIN =[" + tPIN + "])");
		...
		try {
			...
			LogUtil.info(LOGGER,"SampleHashMap::assymEncrypt(EncryptCardNo =["
					+ tEncryptPIN + "])");

			return tEncryptPIN;
		} catch (Exception e) {
			LOGGER.error("assymEncrypt",e);
			return tPIN;
		}
	}
