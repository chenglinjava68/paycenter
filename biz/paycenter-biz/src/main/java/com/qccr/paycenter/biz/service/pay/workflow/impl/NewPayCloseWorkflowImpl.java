...
import com.qccr.paycenter.biz.service.pay.workflow.NewPayCloseWorkflow;
import com.qccr.paycenter.biz.task.close.PayCloseWorkflowCallable;
 ...
 */
@Service
public class NewPayCloseWorkflowImpl implements NewPayCloseWorkflow{
    private static final Logger LOGGER = LoggerFactory.getLogger(NewPayCloseWorkflowImpl.class);
    ...
    public PayCloseResultSO close(final String businessNo, final String partner) throws Exception {
        ...
        if(tradeOrderDO.getStatus() == 1){
            throw new PayCenterException(PayCenterStateCode.PAY_CLOSE_PAYED, "订单支付完成，不可关闭");
        }else if(tradeOrderDO.getStatus() == PayOrderEnum.CLOSE.getValue()){
            PayCloseResultSO resultSO = new PayCloseResultSO();
            resultSO.setBusinessNo(businessNo);
        ...
        ExecutorUtil.close(new PayCloseWorkflowCallable(this,paramSO));
    ...
    private boolean checkThirdTradeStatus(PayOrderDO payOrderDO) throws PayCenterException{
        ...
        propertiesSo.setPayerAccount(payOrderDO.getPayerAccount());
        ...
        }else if(propertiesSo.getChannel().equals(PayConstants.BOCOM_PAY_CHANNEL)) {//交行
            resultSO = bocomPayProcess.queryPayOrder(propertiesSo);
        }
        if(resultSO != null && PayOrderEnum.SUCCESS.equals(resultSO.getPayOrderEnum())) {
            ...
            notifySO.setBillNo(resultSO.getBillNo());//将票据号存到三方流水号里面
            notifySO.setPayNo(resultSO.getPayNo());
            notifySO.setAmount(String.valueOf(payOrderDO.getAmount()));
            notifySO.setPayChannel(resultSO.getChannel());
            notifySO.setPayType(resultSO.getType());
            notifySO.setPartner(resultSO.getPartner());
            notifySO.setPayerAccount(resultSO.getPayerAccount());
            notifySO.setMchId(resultSO.getMchId());
            paySuccess(notifySO, payOrderDO);
     ...
     * @param notifySO
     */
    private void paySuccess(final PayNotifySO notifySO, final PayOrderDO propertiesSo){
        ...
        LogUtil.info(LOGGER, String.valueOf(propertiesSo.getAmount().longValue()));
        if(sumAmount.longValue() == propertiesSo.getAmount().longValue()) {
