...
import com.qccr.paycenter.biz.service.pay.workflow.OldPayCloseWorkflow;
 ...
 */
@Service
public class OldPayCloseWorkflowImpl implements OldPayCloseWorkflow{
    private static final Logger LOGGER = LoggerFactory.getLogger(OldPayCloseWorkflowImpl.class);

    public static final String CODE = "OLD";
    ...
    /**
     * 关闭订单，考虑有多条记录的情况（partner不同），但一般大部分情况还是一条记录的居多
     * @param businessNo
     * @param partner
     * @return
     * @throws Exception
    ...
    public PayCloseResultSO close(final String businessNo, final String partner) throws Exception {
        final List<PayOrderDO> payOrderDOList =  payOrderBO.findByBusinessNo(businessNo);
        ...
        reentrantTransactionBO.execute(new TransactionCallback<Void>() {
            ...
            public Void doTransaction() {
                payOrderBO.payCloseByBizNo(businessNo);//只处理支付订单；支付流水不做处理
                return null;
            }
        });
    ...
    public PayCloseResultSO doClose(PayCloseSO paramSO) throws PayCenterException {
        final List<PayOrderDO> payOrderDOList =  payOrderBO.findByBusinessNo(paramSO.getBusinessNo());
     ...
     * @throws PayCenterException
     */
    public boolean checkThirdTradeStatus(PayOrderDO payOrderDO) throws PayCenterException{
        //查最新的一条流水来判断渠道
